...
## Base R ~~vs.~~ & tidyverse {#baseR-tidyverse}
...
## What exactly does the pipe operator |>? {#what-is-pipe-op}
...
2. Calculate descriptive statistics using the `describe` function from the `psych` package [@R-psych] **and then**
3. Create a table with the `kable` function from the `knitr` package [@R-knitr]
