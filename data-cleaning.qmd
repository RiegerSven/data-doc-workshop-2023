...
This section provides an introduction to data cleaning. When data is really messy, data cleaning can be a challenging task. We provide some approaches that however, will not include or solve all issues during data cleaning.
...
Depending on how much data cleaning is needed, it is recommended to clean data in a sequential way (We like it neat!). This means after we succeed with one step of data cleaning, we save the respective `R`- or `Quarto` script and the data set with an appropriate description. 

::: {.callout-tip collapse="false" appearance="simple" title="Tip: Sequential way of working with data"} 
...
The sequential procedure is also exemplified in the [Remove duplicate observations](#rem-duplicate) section.
...
- location: mean, median &rarr; `mean`, `median`
- dispersion: standard deviation, variance &rarr; `sd`, `var`
